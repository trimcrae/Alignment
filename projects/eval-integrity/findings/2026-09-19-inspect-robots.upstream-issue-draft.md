...
## Companion pull request (draft; file after the issue, or with it)

The verified patch is `repro/inspect_robots/fix-scorer-exception.patch`: the guard in `eval.py` plus a sibling test `test_raising_scorer_degrades_to_error_log` next to the reducer test. Against a clean clone at 7e4d1b7 it passes every gate in `CONTRIBUTING.md`: `ruff check` and `ruff format --check` on the changed files, `mypy` strict, and `pytest --cov` at 100.00 percent with 1721 passed. (A current ruff reports five pre-existing RUF036 hits in files the patch does not touch; the project pins `ruff>=0.6`, so their pinned version may not.)

Changelog entry for `CHANGELOG.md` under "Unreleased", section "Fixed":

```markdown
- **Core:** a scorer that raises no longer crashes `eval()` and loses the run's
  log. The failure degrades the scene and run to `"error"` with
  `scorer '<name>' failed: ...` recorded, the other scorers' values for that
  trial are kept, and the log still reaches disk, matching the reducer path
  and the 0.4.0 "never lose the log" guarantee (#<issue number>).
```

PR title: `eval: degrade a raising scorer to an error log instead of crashing`

PR body, following `.github/PULL_REQUEST_TEMPLATE.md`: Summary (the paragraph above, in short), Changes (guard, test, changelog), the checklist ticked as verified, and `Closes #<issue number>`.
